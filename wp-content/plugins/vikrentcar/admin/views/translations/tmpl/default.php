...
if (!(count($langs) > 1)) {
	...
	?>
	<p class="err"><?php echo JText::translate('VRCTRANSLATIONERRONELANG'); ?></p>
...
<form action="index.php?option=com_vikrentcar&amp;task=translations" method="post" onsubmit="return vrcCheckChanges();">
	<div style="width: 100%; display: inline-block;" class="btn-toolbar vrc-btn-toolbar" id="filter-bar">
		<div class="btn-group pull-right">
			<button class="btn" type="submit"><?php echo JText::translate('VRCGETTRANSLATIONS'); ?></button>
		</div>
		<div class="btn-group pull-right">
			...
			foreach ($xml_tables as $key => $value) {
				...
				?>
				<option value="<?php echo JHtml::fetch('esc_attr', $key); ?>"<?php echo $vrc_tn->replacePrefix($key) == $table ? ' selected="selected"' : ''; ?>><?php echo JHtml::fetch('esc_html', $value); ?></option>
				<?php
			}
			?>
			</select>
	...
	<input type="hidden" name="vrc_lang" class="vrc_lang" value="<?php echo JHtml::fetch('esc_attr', $vrc_tn->default_lang); ?>">
	<input type="hidden" name="option" value="com_vikrentcar" />
...
foreach ($langs as $ltag => $lang) {
	...
	if (!defined('ABSPATH') && is_file(JPATH_SITE . DIRECTORY_SEPARATOR . 'media' . DIRECTORY_SEPARATOR . 'mod_languages' . DIRECTORY_SEPARATOR . 'images' . DIRECTORY_SEPARATOR . $lcountry . '.gif')) {
		...
		?><div class="vrc-translation-tab<?php echo $is_def ? ' vrc-translation-tab-default' : ''; ?>" data-vrclang="<?php echo JHtml::fetch('esc_attr', $ltag); ?>">
		<?php
		if (!empty($flag)) {
			?>
...
if (!defined('ABSPATH')) {
	?>
		<div class="vrc-translation-tab vrc-translation-tab-ini" data-vrclang="">
			...
			<span class="vrc-translation-langname"><?php echo JText::translate('VRCTRANSLATIONINISTATUS'); ?></span>
		</div>
	<?php
}
?>
...
foreach ($langs as $ltag => $lang) {
	...
	if (empty($active_table_key)) {
		?>
			<p class="warn"><?php echo JText::translate('VRCTRANSLATIONSELTABLEMESS'); ?></p>
		<?php
		...
		?>
			<fieldset class="adminform">
				<legend class="adminlegend"><?php echo $active_table; ?> - <?php echo $lang['name'].($is_def ? ' - '.JText::translate('VRCTRANSLATIONDEFLANG') : ''); ?></legend>
				...
				if ($type == 'json') {
					...
					if (count($json_def_values) > 0) {
						foreach ($json_def_values as $jkey => $jval) {
							...
							$json_lbl = '&nbsp;';
							if (!is_numeric($jkey)) {
								$guess_lbl = JText::translate('VRC_' . strtoupper($jkey));
...
function vrcCheckChanges() {
	...
	return confirm("<?php echo addslashes(JText::translate('VRCTANSLATIONSCHANGESCONF')); ?>");
}
