...
<div class="vrc-admin-wizard-container">
	...
	if (!empty($this->rows)) {
		?>
		<a class="btn vrc-wizard-btn" href="index.php?option=com_vikrentcar"><?php VikRentCarIcons::e('home'); ?> <?php echo JText::translate('VRCMENUDASHBOARD'); ?></a>
		<?php
	}
	?>
		<a class="btn vrc-wizard-btn" href="javascript: void(0);" onclick="showVrcWizard();"><?php VikRentCarIcons::e('magic'); ?> <?php echo JText::translate('VRCTOGGLEWIZARD'); ?></a>
	</div>
</div>
<?php
}
					...
					<div class="vrc-quickres-head">
						...
						<div class="vrc-quickres-head-right">
							<form name="vrchcar" method="post" action="index.php?option=com_vikrentcar">
								...
								foreach ($allc as $cc) {
									?>
									<option value="<?php echo (int)$cc['id']; ?>"<?php echo $cc['id'] == $idcar ? ' selected="selected"' : ''; ?>><?php echo JHtml::fetch('esc_html', $cc['name']); ?></option>
									<?php
								}
								?>
								</select>
					...
					<div class="vrc-param-container">
						...
						<div class="vrc-param-setting">
							<div class="vrc-fares-tabs">
								<div class="dailypricesactive"><?php echo JText::translate('VRCDAILYFARES'); ?></div>
								<div class="hourscharges">
									<a href="index.php?option=com_vikrentcar&task=hourscharges&cid[]=<?php echo $idcar; ?>"><?php echo JText::translate('VRCHOURSCHARGES'); ?></a>
								</div>
								<div class="hourlyprices">
									<a href="index.php?option=com_vikrentcar&task=tariffshours&cid[]=<?php echo $idcar; ?>"><?php echo JText::translate('VRCHOURLYFARES'); ?></a>
								</div>
							</div>
						<?php
						if (empty($prices)) {
							...
							<p class="err">
								...
								<a href="index.php?option=com_vikrentcar&task=newprice"><?php echo JText::translate('VRHERE'); ?></a>
							</p>
							<?php
						}
						?>
							<form name="newd" method="post" action="index.php?option=com_vikrentcar" onsubmit="javascript: if (!document.newd.ddaysfrom.value.match(/\S/)){alert('<?php echo addslashes(JText::translate('VRMSGTWO')); ?>'); return false;} else {return true;}">
								<div class="vrc-insertrates-cont">
									<div class="vrc-insertrates-top">
										<div class="vrc-ratestable-lbl"><?php echo JText::translate('VRDAYS'); ?></div>
									...
									<div class="vrc-insertrates-bottom">
										<div class="vrc-ratestable-lbl"><?php echo JText::translate('VRDAILYPRICES'); ?></div>
								...
								<div class="vrc-insertrates-save">
									<input type="submit" class="btn vrc-config-btn" name="newdispcost" value="<?php echo JHtml::fetch('esc_attr', JText::translate('VRINSERT')); ?>"/>
									<input type="hidden" name="cid[]" value="<?php echo JHtml::fetch('esc_attr', $idcar); ?>"/>
									<input type="hidden" name="task" value="tariffs"/>
								</div>
							</form>
				...
				if (empty($rows)) {
					...
					<form action="index.php?option=com_vikrentcar" method="post" name="adminForm" id="adminForm" class="vrc-list-form">
						<div class="vrc-tariffs-updaterates-cont">
							<input type="submit" name="modtar" value="<?php echo JHtml::fetch('esc_attr', JText::translate('VRPVIEWTARTWO')); ?>" onclick="vrRateSetTask(event);" class="btn vrc-config-btn" />
						</div>
						<div class="table-responsive">
							...
							foreach ($tottar as $kt => $vt) {
								...
								<tr class="row<?php echo $k; ?>">
									<td class="left">
										<input type="checkbox" id="cb<?php echo $i;?>" name="cid[]" value="<?php echo JHtml::fetch('esc_attr', $multiid); ?>" onclick="Joomla.isChecked(this.checked);">
									</td>
