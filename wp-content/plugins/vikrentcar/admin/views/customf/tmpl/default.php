...
<div class="table-responsive">
	...
	for ($i = 0, $n = count($rows); $i < $n; $i++) {
		...
		<tr class="row<?php echo $k; ?>">
			...
			<td><a href="index.php?option=com_vikrentcar&amp;task=editcustomf&amp;cid[]=<?php echo (int)$row['id']; ?>"><?php echo JHtml::fetch('esc_html', JText::translate($row['name'])); ?></a></td>
			<td><?php echo ucwords($row['type']).($row['isnominative'] == 1 ? ' <span class="badge">'.JText::translate('VRCISNOMINATIVE').'</span>' : '').($row['isphone'] == 1 ? ' <span class="badge">'.JText::translate('VRCISPHONENUMBER').'</span>' : '').(!empty($row['flag']) ? ' <span class="badge">'.JText::translate('VRCIS'.strtoupper($row['flag'])).'</span>' : ''); ?></td>
