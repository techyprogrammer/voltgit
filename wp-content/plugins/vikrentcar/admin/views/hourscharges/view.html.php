...
class VikRentCarViewHourscharges extends JViewVikRentCar {
	...
	function display($tpl = null) {
			...
			if (empty($phhoursto) || $phhoursfrom == $phhoursto) {
				foreach ($prices as $pr) {
					...
					if (!empty($tmpvarone)) {
						...
						$safeq = "SELECT `id` FROM `#__vikrentcar_hourscharges` WHERE `ehours`=".$dbo->quote($phhoursfrom)." AND `idcar`='".$carrows['id']."' AND `idprice`='".$pr['id']."';";
						$dbo->setQuery($safeq);
						$dbo->execute();
						if ($dbo->getNumRows() == 0) {
							$q = "INSERT INTO `#__vikrentcar_hourscharges` (`idcar`,`ehours`,`idprice`,`cost`) VALUES('".$carrows['id']."',".$dbo->quote($phhoursfrom).",'".$pr['id']."','".($tmpvarone * $phhoursfrom)."');";
							$dbo->setQuery($q);
							$dbo->execute();
						} elseif ($dbo->getNumRows() == 1) {
							$upd_id = $dbo->loadResult();
							$q = "UPDATE `#__vikrentcar_hourscharges` SET `cost`='".($tmpvarone * $phhoursfrom)."' WHERE `id`=".(int)$upd_id." AND `ehours`=".$dbo->quote($phhoursfrom)." AND `idcar`='".$carrows['id']."' AND `idprice`='".$pr['id']."' LIMIT 1;";
							$dbo->setQuery($q);
							$dbo->execute();
						}
					}
				...
				for ($i = intval($phhoursfrom); $i <= intval($phhoursto); $i++) {
					foreach ($prices as $pr) {
						...
						if (!empty($tmpvarone)) {
							$tmpvartwo = VikRequest::getString('hattr'.$pr['id'], '', 'request');
							$multipattr = is_numeric($tmpvartwo) ? true : false;
							$safeq = "SELECT `id` FROM `#__vikrentcar_hourscharges` WHERE `ehours`=".$dbo->quote($i)." AND `idcar`='".$carrows['id']."' AND `idprice`='".$pr['id']."';";
							$dbo->setQuery($safeq);
							$dbo->execute();
							if ($dbo->getNumRows() == 0) {
								$q = "INSERT INTO `#__vikrentcar_hourscharges` (`idcar`,`ehours`,`idprice`,`cost`) VALUES('".$carrows['id']."',".$dbo->quote($i).",'".$pr['id']."','".($tmpvarone * $i)."');";
								$dbo->setQuery($q);
								$dbo->execute();
							} elseif ($dbo->getNumRows() == 1) {
								$upd_id = $dbo->loadResult();
								$q = "UPDATE `#__vikrentcar_hourscharges` SET `cost`='".($tmpvarone * $i)."' WHERE `id`=".(int)$upd_id." AND `ehours`=".$dbo->quote($i)." AND `idcar`='".$carrows['id']."' AND `idprice`='".$pr['id']."' LIMIT 1;";
								$dbo->setQuery($q);
								$dbo->execute();
							}
						}
		...
		$q = "SELECT * FROM `#__vikrentcar_hourscharges` WHERE `idcar`='".$carrows['id']."' ORDER BY `#__vikrentcar_hourscharges`.`ehours` ASC, `#__vikrentcar_hourscharges`.`idprice` ASC;";
		$dbo->setQuery($q);
		$dbo->execute();
	...
	protected function addToolBar() {
		JToolBarHelper::title(JText::translate('VRMAINTARIFFETITLE'), 'vikrentcar');
		JToolBarHelper::save( 'cancel', JText::translate('VRMAINTARIFFEBACK'));
		if (JFactory::getUser()->authorise('core.delete', 'com_vikrentcar')) {
			JToolBarHelper::deleteList(JText::translate('VRCDELCONFIRM'), 'removehourscharges', JText::translate('VRMAINTARIFFEDEL'));
		}
	}
