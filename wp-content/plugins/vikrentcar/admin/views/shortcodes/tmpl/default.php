...
<form action="admin.php" method="post" name="adminForm" id="adminForm">
	...
	<div class="tablenav top">
		...
		<div class="alignleft actions">
			<input type="search" id="post-search-input" name="filter_search" value="<?php echo JHtml::fetch('esc_attr', $this->filters['search']); ?>" />
		...
		<div class="alignright actions" style="padding-right:0;">
			...
			<select name="filter_type" id="vik-type-filter" onchange="document.adminForm.submit();">
				...
				{
					?>
					<option value="<?php echo JHtml::fetch('esc_attr', $type); ?>" <?php echo ($type == $this->filters['type'] ? 'selected="selected"' : ''); ?>><?php echo JHtml::fetch('esc_html', JText::translate($title)); ?></option>
					<?php
				}
				?>
			</select>
			...
			<select name="filter_lang" id="vik-lang-filter" onchange="document.adminForm.submit();" style="margin-right: 0;">
				...
				{
					?>
					<option value="<?php echo JHtml::fetch('esc_attr', $tag); ?>" <?php echo ($tag == $this->filters['lang'] ? 'selected="selected"' : ''); ?>><?php echo JHtml::fetch('esc_html', $lang['nativeName']); ?></option>
					<?php
				}
				?>
			</select>
				...
				<td style="text-align: center;">
					<?php if ($row->post_id) { ?>
						...
						<a href="<?php echo get_permalink($row->post_id); ?>" target="_blank" class="btn btn-primary vrc-link-btn-small">
							<?php echo JText::translate('VRC_SC_VIEWFRONT'); ?> <i class="<?php echo VikRentCarIcons::i('external-link-square'); ?>"></i>
						</a>
					...
					<?php } else if ($row->tmp_post_id) { 
						...
						<a href="edit.php?post_status=trash&post_type=<?php echo $post->post_type; ?>" target="_blank" class="btn vrc-link-btn-small">
							<?php echo JText::translate('VRC_SC_VIEWTRASHPOSTS'); ?> <i class="<?php echo VikRentCarIcons::i('external-link-square'); ?>" style="color: #900;"></i>
						</a>

					<?php } else { ?>

						<a href="index.php?option=com_vikrentcar&task=shortcode.add_to_page&sc_id=<?php echo $row->id; ?>&return=<?php echo $this->returnLink; ?>" class="btn btn-danger vrc-link-btn-small" onclick="return confirm('<?php echo $this->escape(JText::translate('VRC_SC_ADDTOPAGE_HELP')); ?>');">
							<?php echo JText::translate('VRC_SC_ADDTOPAGE'); ?> <i class="<?php echo VikRentCarIcons::i('plus-square'); ?>"></i>
						</a>
