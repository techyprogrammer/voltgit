...
class VikRentCarController extends JControllerVikRentCar {
	...
	public function updatebusy() {
		...
		if (!empty($pidorder)) {
			...
			if ($second > $first) {
				...
				$opertwounits = true;
				$check = "SELECT `id`,`ritiro`,`realback`,`stop_sales` FROM `#__vikrentcar_busy` WHERE `idcar`=" . (int)$pidcar . " AND `id`!=" . $dbo->quote($pidbusy) . " AND `realback` > " . $first . ";";
				$dbo->setQuery($check);
				$dbo->execute();
				if ($dbo->getNumRows() > 0) {
	...
	function setnewrates() {
		// @wponly lite - the seasonal prices function is not available with this version of the framework and will never be called
