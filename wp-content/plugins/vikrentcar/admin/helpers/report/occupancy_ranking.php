...
class VikRentCarReportOccupancyRanking extends VikRentCarReport
{
	...
	function __construct()
	{
		...
		$this->reportName = JText::translate('VRCREPORT'.strtoupper(str_replace('_', '', $this->reportFile)));
	...
	public function getFilters()
	{
		...
		// from Date Filter
		$filter_opt = array(
			'label' => '<label for="fromdate">'.JText::translate('VRCREPORTSDATEFROM').'</label>',
		...
		// to Date Filter
		$filter_opt = array(
			'label' => '<label for="todate">'.JText::translate('VRCREPORTSDATETO').'</label>',
		...
		$periods = array(
			'month' => JText::translate('VRPVIEWRESTRICTIONSTWO'),
			'week' => 'Week',
			'day' => JText::translate('VRDAY'),
		);
		$periods_sel_html = $vrc_app->getNiceSelect($periods, $pperiod, 'period', '', '', '', '', 'period');
		$filter_opt = array(
			'label' => '<label for="period">'.JText::translate('VRROVWSELPERIOD').'</label>',
		...
		if (count($cars)) {
			$cars_sel_html = $vrc_app->getNiceSelect($cars, $pidcar, 'idcar', JText::translate('VRCSTATSALLCARS'), JText::translate('VRCSTATSALLCARS'), '', '', 'idcar');
			$filter_opt = array(
				'label' => '<label for="idcar">'.JText::translate('VRCREPORTSCARFILT').'</label>',
				'html' => $cars_sel_html,
				'type' => 'select',
				'name' => 'idcar'
			);
	...
	public function getReportData()
	{
		...
		if (empty($pfromdate) || empty($from_ts) || empty($to_ts)) {
			$this->setError(JText::translate('VRCREPORTSERRNODATES'));
			return false;
		}
		...
		if (!count($records)) {
			if ($pperiod != 'full') {
				...
				$this->setError(JText::translate('VRCREPORTSERRNORESERV'));
				return false;
			}
		...
		$this->cols = array(
			...
			array(
				'key' => 'day',
				'sortable' => 1,
				'label' => JText::translate('VRCREPORTREVENUEDAY')
			),
			// cars sold
			array(
				'key' => 'cars_sold',
				...
				'sortable' => 1,
				'label' => JText::translate('VRCREPORTREVENUERSOLD'),
			...
			array(
				...
				'sortable' => 1,
				'label' => JText::translate('VRCGRAPHTOTNIGHTSLBL')
			),
			// total bookings
			array(
				'key' => 'tot_bookings',
				...
				'sortable' => 1,
				'label' => JText::translate('VRCREPORTREVENUETOTB')
			),
			// % occupancy
			array(
				'key' => 'occupancy',
				...
				'sortable' => 1,
				'label' => JText::translate('VRCREPORTREVENUEPOCC')
			),
			// ADR
			array(
				'key' => 'adr',
				...
				'sortable' => 1,
				'label' => JText::translate('VRCREPORTREVENUEADR'),
				'tip' => JText::translate('VRCREPORTREVENUEADRHELP')
			),
			// RevPAC
			array(
				'key' => 'revpar',
				...
				'sortable' => 1,
				'label' => JText::translate('VRCREPORTREVENUEREVPAR'),
				'tip' => JText::translate('VRCREPORTREVENUEREVPARH')
			),
			// Taxes
			array(
				'key' => 'taxes',
				...
				'sortable' => 1,
				'label' => JText::translate('VRCREPORTREVENUETAX')
			),
			// Revenue
			array(
				'key' => 'revenue',
				...
				'sortable' => 1,
				'label' => JText::translate('VRCREPORTREVENUEREV')
			)
		);
		...
		array_push($this->footerRow, array(
			array(
				...
				'value' => '<h3>'.JText::translate('VRCREPORTSTOTALROW').'</h3>'
			),
			array(
				'attr' => array(
					'class="center"'
	...
	public function getChart($data = null)
	{
		...
		if ($chart_type != 'line' && count($chart_data) === 1 && is_array($data) && isset($data['keys'])) {
			if (((is_array($data['keys']) && in_array('occupancy', $data['keys'])) || $data['keys'] == 'occupancy')) {
				...
				$chart_labels = array(
					JText::translate('VRCCARSOCCUPANCY'),
					JText::translate('VRCCARSUNSOLD'),
				);
...
jQuery("#' . $canvas_id . '").on("vrc_update_report_chart", function() {
	...
	private function generateChartMetaData($data = null)
	{
		...
		$occ_lbl = trim(str_replace('%', '', JText::translate('VRCREPORTREVENUEPOCC')));
